...
public class LichTrinh_JPanel extends javax.swing.JPanel {
    ...
    public LichTrinh_JPanel() {
       ...
       for(TauEntity tau : dsTau) {
           modelComboBox.addElement(tau.getMaTau());
       }
       cbo_MaTau.setModel(modelComboBox);

       duaDuLieuVaoComboBox(cbo_MaTau, null, null);
    }
    ...
    private void loadDuLieuTuDataLenTable() {
        ...
        for (LichTrinhEntity lt : dsLichTrinh) {
            ...
            model.addRow(new Object[]{
                lt.getMaHT(), 
                lt.getTau().getMaTau(),  // Hiển thị mã tàu
                lt.getNgayDi(),
                lt.getGioDi().toString(), 
                lt.getNgayDen(),
                lt.getGioDen().toString(), 
                trangThai, 
                loai
            });
        }
    ...
    private void table_DanhSachLichTrinhMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_table_DanhSachLichTrinhMouseClicked
        try {
           ...
           SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd"); // Định dạng ngày theo dữ liệu của bạn
           dateNgayDen.setDate(model.getValueAt(row, 4) != null ? sdf.parse(model.getValueAt(row, 4).toString()) : null);
           dateNgayDi.setDate(model.getValueAt(row, 2) != null ? sdf.parse(model.getValueAt(row, 2).toString()) : null);
...
//        timKiemSanPham(dieuKien);
    ...
    private void duaDuLieuVaoComboBox(JComboBox comboBox, ArrayList<?> data, String thuocTinh) {
        ...
        if (comboBox == cbo_MaTau) {
            ArrayList<TauEntity> dsTau = tau_dao.getAllTauDangHoatDong();
            for(TauEntity tau : dsTau) {
                model.addElement(tau.getMaTau());
            }
        } else {
